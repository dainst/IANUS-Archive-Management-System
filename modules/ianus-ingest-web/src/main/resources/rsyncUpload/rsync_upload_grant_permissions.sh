...
# sudo chown root:root rsync_upload_grant_permissions.sh
# sudo chmod 774 rsync_upload_grant_permissions.sh
...
# test (fail): 
# ./rsync_upload_grant_permissions.sh --storage-path=/foo/ipstorage --ip-root=89 --ip-target=/some/path --username=hschmeer

# test (succeed):
# ./rsync_upload_grant_permissions.sh --storage-path=/foo/tip_storage --ip-root=/tp_2017_89_00001289374 --ip-target=/some/path --username=hschmeer --upload-id=3
...
echo "Granting write permissions for user ${username} at location ${absolutePath}"

groupadd -f $groupname
usermod -a -G $groupname $username
chgrp -R $groupname $absolutePath
chmod -R 774 $absolutePath
